...
import type { KillRequirement, CollectItem, ObjectRequirement } from '~/lib/types'
...
export function AdventureForm({ parameters, onChange }: TaskFormProps) {
  const targetEntries = (parameters.targetEntries as number[]) || []
  const objectEntries = (parameters.objectEntries as number[]) || []
  const killRequirements = (parameters.killRequirements as KillRequirement[]) || []
  const objectRequirements = (parameters.objectRequirements as ObjectRequirement[]) || []
  ...
  // Object Requirements
  const updateObjectRequirement = (index: number, field: keyof ObjectRequirement, value: number) => {
    const updated = objectRequirements.map((req, i) =>
      i === index ? { ...req, [field]: value } : req
    )
    onChange({ objectRequirements: updated })
  }

  const addObjectRequirement = () => {
    onChange({ objectRequirements: [...objectRequirements, { entry: 0, count: 1 }] })
  }

  const removeObjectRequirement = (index: number) => {
    onChange({ objectRequirements: objectRequirements.filter((_, i) => i !== index) })
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Object Entries */}
      <div className="space-y-2">
        <Label>Target Object Entries</Label>
        <EntityArrayEditor
          values={objectEntries}
          onChange={(values) => onChange({ objectEntries: values })}
          entityType="object"
          placeholder="Object Entry"
        />
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="space-y-2">
          ...
          <Input
            ...
            type="number"
            min={1}
            value={parameters.searchRadius as number || 100}
            onChange={(e) => onChange({ searchRadius: parseInt(e.target.value) || 100 })}
          />
        </div>
        <div className="space-y-2">
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <Checkbox
            id="defendSelf"
            checked={parameters.defendSelf as boolean ?? true}
            onCheckedChange={(checked) => onChange({ defendSelf: checked === true })}
          />
          <Label htmlFor="defendSelf" className="cursor-pointer">
            Defend Self (react to attackers)
          </Label>
        </div>
        <div className="flex items-center gap-2">
          <Checkbox
            id="objectsGiveLoot"
            checked={parameters.objectsGiveLoot as boolean || false}
            onCheckedChange={(checked) => onChange({ objectsGiveLoot: checked === true })}
          />
          <Label htmlFor="objectsGiveLoot" className="cursor-pointer">
            Objects Give Loot (wait for loot window)
          </Label>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {killRequirements.map((req, index) => (
            <div key={index} className="flex gap-2 items-start">
              ...
              <Button variant="ghost" size="sm" className="text-destructive mt-5" onClick={() => removeKillRequirement(index)}>
                Remove
              </Button>
            </div>
          ))}
      ...
      {/* Object Requirements */}
      <div className="space-y-2">
        <Label>Object Use Requirements</Label>
        <div className="space-y-2">
          {objectRequirements.map((req, index) => (
            <div key={index} className="flex gap-2 items-start">
              <div className="flex-1">
                <Label className="text-xs">Object Entry</Label>
                <EntityInput
                  type="object"
                  value={req.entry || 0}
                  onChange={(value) => updateObjectRequirement(index, 'entry', value)}
                />
              </div>
              <div className="w-24 space-y-1">
                <Label className="text-xs">Count</Label>
                <Input
                  ...
                  min={1}
                  value={req.count || ''}
                  onChange={(e) => updateObjectRequirement(index, 'count', parseInt(e.target.value) || 1)}
                />
              </div>
              <Button variant="ghost" size="sm" className="text-destructive mt-5" onClick={() => removeObjectRequirement(index)}>
                Remove
              </Button>
            </div>
          ))}
          <Button variant="outline" size="sm" onClick={addObjectRequirement}>
            + Add Object Requirement
          </Button>
        </div>
      </div>
          ...
          {collectItems.map((item, index) => (
            <div key={index} className="p-3 border rounded-lg space-y-2">
              <div className="flex gap-2 items-start">
                ...
                <Button variant="ghost" size="sm" className="text-destructive mt-5" onClick={() => removeCollectItem(index)}>
                  Remove
                </Button>
              </div>
              <div className="space-y-1">
                <Label className="text-xs">Dropped By (mob or object entries)</Label>
                <EntityArrayEditor
